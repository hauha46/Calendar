...
public class SwingUI {
  ...
  /**
   * Constructs a new SwingUI with an injected controller.
   *
   * @param controller the SwingController instance to be used
   */
  public SwingUI(SwingController controller) {
  ...
  private void setupBottomPanel() {
    ...
    //JLabel statusLabel = new JLabel("Ready");//keep the button here to go to the commandline UI;
    JButton switchToCliButton = new JButton("Switch to CLI");
    switchToCliButton.addActionListener(e -> {
      // Launch the command-line interface in a new thread.
      new Thread(() -> {
        // Use the same CalendarManager instance from the SwingController.
        CommandController cmdController = new CommandController(controller.getCalendarManager(), new Interpreter());
        cmdController.start();
      }).start();
    });
    bottomPanel.add(switchToCliButton, BorderLayout.WEST);
  ...
  /**
   * Static factory method to create and show the GUI.
   *
   * @param controller the SwingController instance
   * @return the created SwingUI instance
   */
  public static SwingUI createAndShowGUI(SwingController controller) {
    // It is assumed that this method is called on the EDT.
    return new SwingUI(controller);
  }
